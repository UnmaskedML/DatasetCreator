...
fieldnames = ['img', 'img_height', 'img_width', 'face_xtl', 'face_ytl', 'face_xbr', 'face_ybr', 'mask_xtl', 'mask_ytl',
              'mask_xtm', 'mask_ytm', 'mask_xtr', 'mask_ytr', 'mask_xbr', 'mask_ybr', 'mask_xbm', 'mask_ybm', 'mask_xbl', 'mask_ybl']
...
def download_dlib_model():
    ...
    with requests.get(dlib_model_link, stream=True) as r:
        print("Zip file size: ", np.round(
            len(r.content) / 1024 / 1024, 2), "MB")
        destination = (
...
def get_six_points(face_landmark, image):
    ...
    nose_mid_line, _, _, _, _ = get_line(
        face_landmark, image, type="nose_long")
    ...
    # cv2.waitKey(0)
    nose_mid_line, _, _, _, _ = get_line(
        face_landmark, image, type="bottom_lip")
    points = get_points_on_chin(nose_mid_line, face_landmark)
    face_d = points[0]
    face_f = points[-1]
...
def mask_face(image, face_location, six_points, angle, args, type="surgical"):
    ...
    # REE
    # print(dst_mask_points)
    for point in dst_mask_points:
        csv_row.append(int(point[0][0]))
        csv_row.append(int(point[0][1]))
    ...
    if debug:
        for i in six_points:
            cv2.circle(out_img, (i[0], i[1]), radius=4,
...
def mask_image(f, image_path, args):
    # Read the image
    ...
    for (i, face_location) in enumerate(face_locations):
        ...
        csv_row.append(height)
        csv_row.append(width)
        csv_row.append(face_location.left())
        csv_row.append(face_location.top())
        csv_row.append(face_location.right())
        csv_row.append(face_location.bottom())
